...
import vf.prototyper.view.{Component, Icon}
...
class EditVc(original: Vector[View])
{
	...
	private val viewsPointer = new EventfulPointer(original.map(ViewBuilder.apply))
	
	private var firstView = views.head
	
	private val currentViewPointer = new EventfulPointer(firstView)
	private val currentLinkPointer = EventfulPointer.empty[LinkBuilder]()
	
	private val viewNameInputPointer = new EventfulPointer(currentView.name)
	private val isFirstViewPointer = new EventfulPointer(true)
		...
		val window = ReachWindow.withContext(context.window.withAnchorAlignment(Top))
			.using(ViewStack, title = "Prototyper") { (_, stackF) =>
				...
				stackF.withoutMargin.build(Mixed) { factories =>
					...
					val header = factories.next()(Framing).small.withBackground(primary.dark).build(Stack) { stackF =>
						stackF.row.build(Mixed) { factories =>
							...
							val arrows = Pair(Icon.arrowLeft, Icon.arrowRight).mapWithSides { (icon, end) =>
								factories(ImageButton)
									.withInsets(StackInsets.any.mapVertical { _.withLowPriority })
							...
							// Name field
							val nameField = factories(TextField).withFieldName("View Name")
								.string(length.field.medium.upscaling, textPointer = viewNameInputPointer)
							// Is first -flag
							val isFirstFlag = factories(CheckBox.usingIcons(Pair(Icon.oneFilled.large, Icon.oneEmpty.large)))
								.withEnabledPointer(!isFirstViewPointer).apply(isFirstViewPointer)
							// Delete button
							// TODO: Add enabled state
							val deleteButton = factories(ImageButton).icon(Icon.delete.large, Some(ColorRole.Failure)) {
								if (views.hasSize > 1) {
									// Deletes the view
					...
					val linkArea = factories.next()(Framing).withBackground(primary.default).expandingToRight
						.build(Stack) { stackF =>
							stackF.related.row.build(Mixed) { factories =>
								val targetDd = Component.dropDownFrom(factories)
									.withFieldName("Link target").withPrompt("Select a view")
